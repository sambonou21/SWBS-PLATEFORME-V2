...
@section('content')
    <h1>Ajouter un projet au portfolio</h1>

    <form method="POST" action="{{ route('admin.portfolio.store') }}" enctype="multipart/form-data" class="swbs-form">
        @csrf
        ...
        <div class="swbs-form-group">
            ...
            <input id="excerpt" name="excerpt" type="text" value="{{ old('excerpt') }}" required>
        ...
        <div class="swbs-form-group">
            ...
            <select id="service_type" name="service_type" required>
                @foreach($services as $service)
                    <option value="{{ \Illuminate\Support\Str::slug($service->title) }}">{{ $service->title }}</option>
                @endforeach
            </select>
